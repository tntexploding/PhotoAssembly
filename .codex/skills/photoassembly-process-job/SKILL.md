...
description: Process pending PhotoAssembly image-stylization jobs with Codex image capabilities. Use when asked to handle a PhotoAssembly job, transform a photo according to a generated treatment plan, or complete a job in the project's configured local jobs directory.
---

# Process a PhotoAssembly job

1. Identify the job ID from the request. If absent, run `node scripts/local-admin.mjs jobs --pending --json` and choose the oldest pending job. This command honors the same configured absolute data directory as the server.
...
8. Report the job ID, the absolute final `result.<ext>` path shown by the completion script, final prompt, and whether a revision was needed.
