...
test('imports root and nested GitHub skills from their live repositories', { skip: process.env.RUN_EXTERNAL_SKILL_TESTS !== '1' }, async t => {
  const jobsDir = await mkdtemp(join(tmpdir(), 'photoassembly-external-jobs-')); t.after(() => rm(jobsDir, { recursive: true, force: true }));
  const jobs = new CodexJobStore({ jobsDir });
  for (const url of sources) {
    ...
    const job = await jobs.create({ imageDataUrl: png, styleId: style.id });
    try {
      const directory = jobs.directory(job.id); await access(join(directory, 'input.png')); await access(join(directory, 'CODEX_TASK.md'));
    ...
    } finally { await jobs.delete(job.id); }
  }
  ...
  for (const style of nested) {
    ...
    const job = await jobs.create({ imageDataUrl: png, styleId: style.id });
    try {
      const manifest = JSON.parse(await readFile(join(jobs.directory(job.id), 'job.json'), 'utf8'));
